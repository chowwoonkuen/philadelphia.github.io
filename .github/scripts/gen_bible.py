...
<body>
  <div class="page">
    ...
    <div class="stat-bar">
      <div class="stat-item"><span class="stat-dot stat-avail"></span><span>{s_uploaded} <strong>{len(existing)}</strong> / 66 卷</span></div>
      <div class="stat-item"><span class="stat-dot stat-miss"></span><span>{s_not_upload}</span></div>
    </div>
'''
...
def render_testament(heading, cat_list):
    ...
    for cat in cat_list:
        ...
        for (num, abbr, name, _) in books:
            if num in existing:
                url = f"{base}/viewer.html?pdf={existing[num]}&v={ver}"
                out += f'          <a href="{url}" target="_blank" class="book-tile avail" title="{name}（{s_click_read}）">\n'
            else:
                out += f'          <div class="book-tile miss" title="{name}（{s_not_avail}）">\n'
